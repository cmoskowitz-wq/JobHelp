...
def filter_seen(jobs: List[Job]) -> List[Job]:
    """
    Filter out jobs seen in the last 24h (today OR yesterday).
    Fixes the cross-day duplicate gap: the first 7am run now correctly
    blocks jobs that were emailed in yesterday evening's runs.
    """
    today = date.today().isoformat()
    ...
    try:
        ...
        for job in jobs:
            ...
            if not conn.execute(
                "SELECT 1 FROM seen_jobs WHERE job_key = ? AND seen_date IN (?, ?)",
                (key, today, yesterday),
            ).fetchone():
                new.append(job)
...
def reset_today() -> None:
    """Clear today's seen_jobs and run history. Useful for re-testing without waiting for tomorrow."""
    today = date.today().isoformat()
    conn = _connect()
    try:
        conn.execute("DELETE FROM seen_jobs WHERE seen_date = ?", (today,))
        conn.execute("DELETE FROM runs WHERE run_date = ?", (today,))
        conn.commit()
        logger.info("[state] Reset today's state (%s) — seen_jobs and runs cleared.", today)
    finally:
        conn.close()
